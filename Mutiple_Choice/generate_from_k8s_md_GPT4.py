...
def wait_until_color_changes(x, y, target_color=(255, 255, 255), timeout=120):
    """等待直到屏幕上的特定像素颜色变化或超时"""
    ...
    for root, dirs, files in os.walk(input_folder):
        for file_name in files:
            if file_name.endswith('.md'):
                ...
                # 出现网络故障，无法发送消息
                if wait_until_image_appears(generating_button, timeout=10) == False:
                    print("Error occur because of network")
                    return 1
                # GPT4.0 回答已经达到上限
                if wait_until_image_appears(max_time, timeout=10) == True:
